...
Currently, the only way to communicate with the kernel debugger is over the serial port. For future versions, we will add other IRQL-compatible mechanisms to communicate.
...
The mechanism to pause the debuggee is also implemented in user-mode. It is because we are not trying to handle each pause request from the debugger in polling mode as this is not an optimized way.
