...
Starts a program with the specific parameters and breaks when the PE file reached the entrypoint.
...
First, you should create a process in the suspended state, then you should fill the **ProcessId** and **ThreadId**, set the **IsStartingNewProcess** to `TRUE`, and Action to `DEBUGGER_ATTACH_DETACH_USER_MODE_PROCESS_ACTION_ATTACH` in the above structure when the IOCTL returns from the kernel, other parts of this structure are filled with appropriate data from the process.
