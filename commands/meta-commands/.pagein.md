...
The following command injects a page-fault with the error code equal to `zero` and starting from a range where its **CR2** register is configured to `00007ff8349f2224`. and the last address is `00007ff8349f2224+(4*0x1000)` which means it brings **5** pages into the memory or the length is `0x4000` bytes.
