...
First of all, you should fill the following structure, set the `ProcessId` or `Process` to your target process (if you want to change the current process), set the `ActionType` to `DEBUGGEE_DETAILS_AND_SWITCH_PROCESS` and leave the `Result`.
...
typedef enum _DEBUGGEE_DETAILS_AND_SWITCH_PROCESS_TYPE
{
    ...
    DEBUGGEE_DETAILS_AND_SWITCH_PROCESS,
