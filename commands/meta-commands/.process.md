...
By default, HyperDbg is spinning on vmx-root, and the default process memory layout is the system process `pid = 4`. Thus, the **cr3** is the same as the system process's **cr3** (the current process might be anything, it's only about the memory layout like you might be in `process id = 0x864` while the **cr3** is the system process's **cr3**). When you change the process using the '**.process**' command, HyperDbg is spinning on vmx-root with the target process's **kernel cr3** (not the system's **cr3**).
