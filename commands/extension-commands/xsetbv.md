---
description: Description of the '!xsetbv' command in HyperDbg.
---

# !xsetbv (hook XSETBV instruction execution)
...
```c
HyperDbg> !xsetbv pid 490 core 2
```
...
```
HyperDbg> !xsetbv script { HyperDbg Script Here }
```

The above command when messages don't need to be delivered immediately.

```
HyperDbg> !xsetbv script { HyperDbg Script Here } imm no
```
...
```
HyperDbg> !xsetbv script {file:c:\users\sina\desktop\script.txt}
```
...
Monitoring process id **0x490** for **XSETBV** instruction execution and run 3 nops whenever the event is triggered. Take a look at [Run Custom Code](https://docs.hyperdbg.org/using-hyperdbg/prerequisites/how-to-create-an-action#run-custom-codes) for more information.

```c
HyperDbg> !xsetbv pid 490 code {90 90 90}
```

Or if you want to use assembly codes directly, you can add an `asm` before the `code`.

```clike
HyperDbg> !xsetbv pid 490 asm code {nop; nop; nop}
```

**Run Custom Code (Conditional)**

Monitoring process id **0x490** for **XSETBV** instruction execution and run 3 nops whenever the event condition is triggered and runs 3 nops whenever the event is triggered. Take a look at [Run Custom Code](https://docs.hyperdbg.org/using-hyperdbg/prerequisites/how-to-create-an-action#run-custom-codes) and [how to create a condition](https://docs.hyperdbg.org/using-hyperdbg/prerequisites/how-to-create-a-condition) for more information.

```c
HyperDbg> !xsetbv pid 490 code {90 90 90} condition {90 90 90}
```

Or if you want to use assembly codes directly, you can add an `asm` before the `condition` and also before the `code`.

```clike
HyperDbg> !xsetbv pid 490 asm code {nop; nop; nop} asm condition {nop; nop; nop}
```
...
This command uses **XSETBV** (**VMX\_EXIT\_REASON\_EXECUTE\_XSETBV - 0x37**) vm-exit to implement **XSETBV** hooks.
