...
> !syscall \[SyscallNumber (hex)] \[pid ProcessId (hex)] \[core CoreId (hex)] \[imm IsImmediate (yesno)] \[sc EnableShortCircuiting (onoff)] \[stage CallingStage (prepostall)] \[buffer PreAllocatedBuffer (hex)] \[script { Script (string) }] \[asm condition { Condition (assembly/hex) }] \[asm code { Code (assembly/hex) }] \[output {OutputName (string)}]
>
> !syscall2 \[SyscallNumber (hex)] \[pid ProcessId (hex)] \[core CoreId (hex)] \[imm IsImmediate (yesno)] \[sc EnableShortCircuiting (onoff)] \[stage CallingStage (prepostall)] \[buffer PreAllocatedBuffer (hex)] \[script { Script (string) }] \[asm condition { Condition (assembly/hex) }] \[asm code { Code (assembly/hex) }] \[output {OutputName (string)}]
...
Please note that **!syscall** at the '**post**' stage indicates you are inside the kernel's system call handler (It does **NOT** mean that you passed the execution of the SYSCALL instruction). If your goal is to alter the result of a system call without actually executing it, you can [short-circuit](https://docs.hyperdbg.org/tips-and-tricks/misc/event-short-circuiting) the SYSCALL event (making it appear as though it ran when it did not) and then modify the return value (for example, by setting the `@rax` register to your desired result). Alternatively, you can use the '[!sysret](https://docs.hyperdbg.org/commands/extension-commands/sysret)' command for changing the return results.
...
{% hint style="danger" %}
This command is not PatchGurad compatible, which means that PatchGuard detects this command and will cause BSOD; thus, make sure to turn it off (e.g., by using the [EfiGuard](https://github.com/mattiwatti/efiguard) project or attaching a kernel-mode WinDbg debugger at the start of the Windows) before using this command. Disabling Driver Signature Enforcement alone won't turn off the PatchGuard.
{% endhint %}
